...
partition_workstation_disk() {
	...
	# ESP
	if ! ui_spin "Creating EFI Partition on ${disk}" sgdisk -n 1:2048:+512M -t 1:EF00 -c 1:'EFI System' "${disk}" 2>>"${LOG_FILE}"; then
		error_exit "Falha ao criar partição EFI em ${disk}"
	fi
	...
	if ! ui_spin "Creating ZFS Partition on ${disk}" sgdisk -n 2:0:0 -t 2:BF00 -c 2:'ZFS Root' "${disk}" 2>>"${LOG_FILE}"; then
		error_exit "Falha ao criar partição ZFS em ${disk}"
	fi

	# Probe
	ui_spin "Probing partitions on ${disk}" partprobe "${disk}" 2>>"${LOG_FILE}" || error_exit "Falha ao executar partprobe em ${disk}"
...
partition_server_disk() {
	...
	# ESP
	if ! ui_spin "Creating EFI Partition on ${disk}" sgdisk -n 1:2048:+256M -t 1:EF00 -c 1:'EFI System' "${disk}" 2>>"${LOG_FILE}"; then
		error_exit "Falha ao criar partição EFI em ${disk}"
	fi
	sgdisk -A 1:set:2 "${disk}" 2>>"${LOG_FILE}" || error_exit "Falha ao definir atributo Legacy Boot na ESP"
...
prepare_disks() {
	...
	ui_spin "Triggering UDEV events" udevadm trigger 2>>"${LOG_FILE}" || log_warn "Falha ao executar udevadm trigger"
	ui_spin "Waiting for UDEV settle" udevadm settle 2>>"${LOG_FILE}" || log_warn "Falha ao executar udevadm settle"
