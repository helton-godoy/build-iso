#!/usr/bin/env bash
#
# install-manager.sh - Orquestrador principal do Aurora OS Installer
...
# Screen de boas-vindas com design Aurora
welcome_screen() {
...
EOF
	...
	**ZFS-on-Root** e **ZFSBootMenu**.
	...
	log_info "Iniciando instalação Aurora OS..."
}
...
collect_system_info() {
	...
	HOSTNAME=$("${gum_path}" gum input \
		--prompt " 🏷️  Hostname: " \
		--placeholder "Ex: aurora" --value "aurora")
...
execute_components() {
	...
	if ! source "components/01-validate.sh"; then
	...
	if ! source "components/02-partition.sh"; then
	...
	if ! source "components/03-pool.sh"; then
	...
	if ! source "components/04-datasets.sh"; then
	...
	if ! source "components/05-extract.sh"; then
	...
	if ! source "components/06-chroot-configure.sh"; then
	...
	if ! source "components/07-bootloader.sh"; then
	...
	if ! source "components/08-cleanup.sh"; then
	...
	if [[ ${failed} -eq 0 ]]; then
		...
		gum format -- "🎉 Instalação Aurora OS concluída!"
		return 0
	else
...
main() {
	...
	gum format \
		...
		--height 8 \
		-- "
	🎉 Instalacao Aurora OS concluida com sucesso!
