...
BIOS_LOG_FILE="${PROJECT_DIR}/logs/test_qemu_bios.log"
...
test_bios() {
  ...
  timeout 60 qemu-system-x86_64 \
    ...
    -serial mon:stdio \
    -no-reboot \
    2>&1 | tee -a "$LOG_FILE" -a "$BIOS_LOG_FILE" | head -100 &
  ...
  if kill -0 $vm_pid 2>/dev/null; then
    ...
    if grep -Eq 'SeaBIOS|Booting from|SYSLINUX' "$BIOS_LOG_FILE"; then
      log "✓ Evidência de trilha de boot BIOS detectada no log"
    else
      error "✗ Sem evidência de trilha de boot BIOS no log"
      return 1
    fi
