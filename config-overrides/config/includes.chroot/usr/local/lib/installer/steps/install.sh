...
step_install() {
  ...
  export INSTALL_TARGET_ROOT_DEFAULT
  export INSTALL_ESP_MOUNT_DEFAULT
  export INSTALL_EFI_MIB_DEFAULT
...
install_validate_topology_disk_count() {
  ...
  raidz10)
    if ((count < 4)); then
      ui_error "Topologia raidz10 requer ao menos 4 discos."
      return 1
    fi
    if ((count % 2 != 0)); then
      ui_error "Topologia raidz10 requer número par de discos."
      return 1
    fi
    ;;
  raidz | raidz1)
