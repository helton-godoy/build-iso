  ...
  if [[ -n "$part_efi" ]]; then
    ...
    if bootutils_is_uefi; then
      ui_process_step "Configurando Boot Entry (EFIBootMgr)..." \
        zbm_configure_efi "$disk" "$efi_num" "$target" || {
        ui_error "Falha ao configurar entrada UEFI via efibootmgr"
        return 1
      }
    else
      ui_process_step "Configurando Boot BIOS (Syslinux)..." \
        zbm_configure_bios "$disk" "$part_efi" "$target" || {
        ui_error "Falha ao configurar boot BIOS legado"
        return 1
      }
    fi
