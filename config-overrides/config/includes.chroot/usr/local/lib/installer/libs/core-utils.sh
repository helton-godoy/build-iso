...
log_init() {
  local lf ld
  lf="${LOG_FILE:-/var/log/fileserver-installer/installer.log}"
  ld="${LOG_DIR:-$(dirname "$lf") }"
...
log_line() {
  ...
  printf '%s [%s] %s\n' "$ts" "$level" "$msg" >>"$lf" 2>/dev/null || true
}
...
die() {
  local msg
  msg="$(sanitize_ws "${1-Erro desconhecido.}")"
  LAST_ERROR="$msg"
  log_line "ERROR" "$msg"
  stderr "$msg"
  return 1
