...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
	...
	Funções disponíveis:
	  system_set_hostname <hostname> [target]
	  system_create_user <user> <pass> [target]
	  system_set_root_password <pass> [target]
	  system_set_timezone [timezone] [target]
	  system_set_locales [target]
	  system_configure_network [target]
	  system_generate_hostid [target]
	  system_configure_fstab <pool> [target]
	"
fi
