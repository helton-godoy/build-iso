...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
	echo "
	=== Teste de Instalação ZFSBootMenu ===

	Funções disponíveis:
	  zbm_install <esp_partition> [target] [pool]
	  zbm_configure_efi <disk> <part_num> [target]
	  zbm_configure_zfs_properties [pool] [hostid]
	  zbm_generate_hostid [target]
	  zbm_setup_fallback <esp_partition> [target]
	  zbm_verify [target]
	"
fi
