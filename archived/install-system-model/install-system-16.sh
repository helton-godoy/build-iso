...
# Module: step/install
# Handler: step_install
# Single responsibility: execute base installation pipeline (disk -> ZFS -> debootstrap -> kernel/ZFS pkgs)
...
step_install() {
    ...
    # swap sizing policy: default off (set via state later if needed)
    ...
    # Ensure essential mountpoints exist
    mkdir -p "$target_root/dev" "$target_root/proc" "$target_root/sys" "$target_root/run" || true
    ...
    # Kernel cmdline baseline: ensure ZFS root is discoverable
    local zfs_root_hint
    ...
    # Leave actual bootloader installation to boot/post_install steps;
    # but ensure tools exist when GRUB selected.
    if [[ "$bootloader" == "grub" ]]; then
        if ! installutils_apt_install_packages "$target_root" "grub-efi-amd64 shim-signed"; then
            # BIOS fallback if not UEFI; keep best-effort
            if ! bootutils_is_uefi; then
    ...
    # Cleanup bind mounts (ESP kept mounted for later steps)
    installutils_bind_umounts "$target_root" "" || true
