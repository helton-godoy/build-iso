...
# Module: step/post_install
# Handler: step_post_install
# Single responsibility: configure target OS (identity, locale, timezone, users, ZFS cache/initramfs) and install bootloader
...
step_post_install() {
    ...
    # Ensure ESP mounted in target if UEFI
    if bootutils_is_uefi; then
        if [[ -z "$efi_part" ]]; then
            # best-effort detect ESP on disk
    ...
    # Final grub config update if grub exists
