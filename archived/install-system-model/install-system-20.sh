...
ui_hero() {
    ...
    if ui_has_gum; then
        "$GUM_BIN" style --foreground "$(ui_theme_primary)" --bold --padding "1 2" --border "rounded" "$title" >/dev/null
        "$GUM_BIN" style --foreground "$(ui_theme_muted)" --padding "0 2" "$subtitle" >/dev/null
        return 0
    fi
...
ui_section() {
    ...
    if ui_has_gum; then
        "$GUM_BIN" style --bold --foreground "$(ui_theme_primary)" --padding "0 1" "▌ $title" >/dev/null
        return 0
    fi
    stdout ""
    stdout "== $title =="
...
ui_error() {
    ...
    log_line "ERROR" "$msg"
    if ui_has_gum; then
        "$GUM_BIN" style --foreground "$(ui_theme_danger)" --bold "${UI_X} $msg" >/dev/null
        return 0
    fi
    ui_plain_error "$msg"
}
...
ui_warn() {
    ...
    log_line "WARN" "$msg"
    if ui_has_gum; then
        "$GUM_BIN" style --foreground "$(ui_theme_warn)" --bold "${UI_WARN} $msg" >/dev/null
        return 0
    fi
    ui_plain_warn "$msg"
}
...
ui_success() {
    ...
    log_line "INFO" "$msg"
    if ui_has_gum; then
        "$GUM_BIN" style --foreground "$(ui_theme_success)" --bold "${UI_OK} $msg" >/dev/null
        return 0
    fi
...
ui_confirm() {
    ...
    if ui_has_gum; then
        "$GUM_BIN" confirm "$prompt" --affirmative="$ok" --negative="$cancel" >/dev/null
        return $?
    fi
...
ui_select() {
    ...
    if ui_has_gum; then
        "$GUM_BIN" choose --header "$prompt" "$@" 2>/dev/null || true
        return 0
    fi
    ui_plain_note "$prompt"
    local i=1 opt
    for opt in "$@"; do
        stdout "  [$i] $opt"
        i=$((i + 1))
...
register_step() {
    ...
    if [[ -z "$step_id" || -z "$order" || -z "$title" || -z "$module" || -z "$handler" ]]; then
...
step_review() {
    ...
    if ui_confirm "Voltar ao menu?" "Voltar" "Sair"; then
        return 0
    fi
    return 1
}
...
boot_register_steps_default() {
    # Minimal, can be expanded to include all screens; lazy loading will still work.
    ...
    register_step "post_install" "70" "Configurações Pós-Instalação"      "Hostname/locale/users/services/boot" "install" "conclusion" \
        "boot-utils.sh,install-utils.sh,post-utils.sh,time-utils.sh,validate-utils.sh,finalize-chroot.sh,setup-zbm-config.sh" \
    ...
    register_step "review"       "50" "Resumo da Configuração"            "Verificar estado e configurações" "" "install" \
        "" \
        "" "step_review"
}
...
menu_loop() {
    ...
    while true; do
        ...
        local entries choice step_id
        entries="$(menu_main_entries)" || return 1

        if ui_has_gum && [[ "$NONINTERACTIVE" != "1" ]]; then
            choice="$("$GUM_BIN" choose --header "Selecione uma etapa" --height 15 $(printf '%q ' $(printf '%s\n' "$entries")) 2>/dev/null || true)"
        else
            choice="$(ui_select "Selecione uma etapa" $(printf '%s\n' "$entries" | sed -E '/^[[:space:]]*$/d'))"
        fi
...
main() {
    ...
    # Ensure review handler exists (built-in)
    STEP_HANDLER["review"]="step_review"
    STEP_TITLE["review"]="${STEP_TITLE[review]-Resumo da Configuração}"
    STEP_ORDER["review"]="${STEP_ORDER[review]-50}"
