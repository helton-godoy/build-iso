...
check_dependencies() {
    ...
    if ! command -v qemu-system-x86_64 &> /dev/null; then
        echo -e "${RED}[ERRO]${NC} qemu-system-x86_64 não encontrado. Preciso instalar o pacote 'qemu-system-x86'."
        missing=$((missing + 1))
    else
        echo -e "${GREEN}[INFO]${NC} Instalando dependências..."
        apt-get install -qqy qemu-system-x86
    fi
