...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
	...
	echo
	echo "Esta é uma operação DESTRUTIVA."
	echo "Use apenas em VMs ou discos de teste."
	echo
	echo "Funções disponíveis:"
	echo "  partition_wipe_disk <disk>"
	echo "  partition_create_gpt <disk>"
	echo "  partition_format_esp <partition>"
	echo "  partition_verify <disk>"
fi
