...
# @DEV_INPUT: $1=mode (uefi|bios), VM_CMD, VM_IP, SSH_USER, SSH_PASSWORD
# @DEV_OUTPUT: Conexão SSH ou serial estabelecida
...
if [[ -n "${VM_CMD:-}" ]]; then
	if ((${#runner[@]} > 0)); then
		env VM_IP="${VM_IP:-}" USE_SSH=true \
			sudo -n --preserve-env=VM_IP,USE_SSH "$SCRIPT_DIR/vm-connect-ssh.sh" "$MODE" "$VM_CMD"
	else
		env VM_IP="${VM_IP:-}" USE_SSH=true "$SCRIPT_DIR/vm-connect-ssh.sh" "$MODE" "$VM_CMD"
	fi
else
	if ((${#runner[@]} > 0)); then
		env VM_IP="${VM_IP:-}" sudo -n --preserve-env=VM_IP "$SCRIPT_DIR/vm-connect-ssh.sh" "$MODE"
	else
		env VM_IP="${VM_IP:-}" "$SCRIPT_DIR/vm-connect-ssh.sh" "$MODE"
	fi
fi
